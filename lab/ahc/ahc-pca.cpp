...
double (*distance_fptr)(double **, const int *, const int *, int, int);
...
double euclidean_distance(const coord_t *a, const coord_t *b) {
  double res = 0.0;
  double na = 0, nb = 0;
  f(i, 0, 1000) {
    res += a->a[i] * b->a[i];
  ...
  return 2 - res / (sqrt(na) * sqrt(nb));
  // f(i, 0, 16) {
  //   res += pow(a->a[i] - b->a[i], 2);
  // }
  // return sqrt(res);
  // return sqrt(pow(a->x - b->x, 2) + pow(a->y - b->y, 2));
}

void fill_euclidean_distances(double **matrix, int num_items,
            const item_t items[]) {
...
double single_linkage(double **distances, const int a[],
...
double complete_linkage(double **distances, const int a[],
           const int b[], int m, int n) {
  double d, max = 0.0 /* assuming distances are positive */;
  for (int i = 0; i < m; ++i)
    for (int j = 0; j < n; ++j) {
      d = distances[a[i]][b[j]];
      if (d > max)
...
double average_linkage(double **distances, const int a[],
          const int b[], int m, int n) {
  double total = 0.0;
  for (int i = 0; i < m; ++i)
    for (int j = 0; j < n; ++j)
...
double centroid_linkage(double **distances, const int a[],
           const int b[], int m, int n) {
...
void print_cluster_node(cluster_t *cluster, int index) {
  return;
  cluster_node_t *node = &(cluster->nodes[index]);
  fprintf(stdout, "Node %d - height: %d, centroid:\n", index, node->height);
  fprintf(stdout, "(");
  f(i, 0, 1000) fprintf(stdout, " %lf", node->centroid.a[i]);;
  fprintf(stdout, ")\n");
...
void merge_items(cluster_t *cluster, cluster_node_t *node,
  ...
  int k = 0, idx;
  coord_t centroid;
  memset(centroid.a, 0, sizeof centroid.a);
...
int read_items(int count, item_t *items, FILE *f) {
  for (int i = 0; i < count; ++i) {
    item_t *t = &(items[i]);
    // TODO
    // if (fscanf(f, "%[^|]| %10f %10f\n", t->label, &(t->coord.x), &(t->coord.y))) continue;
    fscanf(f, "%s", t->label);
    f(j, 0, 1000) fscanf(f, "%lf", &(t->coord.a[j]));
    continue;
    // if (fscanf(f, "%[^|]| %10f %10f\n", t->label, &(t->coord.x), &(t->coord.y))) continue;
    read_fail("item line");
    return i;
